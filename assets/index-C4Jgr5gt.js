(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))d(a);new MutationObserver(a=>{for(const o of a)if(o.type==="childList")for(const m of o.addedNodes)m.tagName==="LINK"&&m.rel==="modulepreload"&&d(m)}).observe(document,{childList:!0,subtree:!0});function s(a){const o={};return a.integrity&&(o.integrity=a.integrity),a.referrerPolicy&&(o.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?o.credentials="include":a.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function d(a){if(a.ep)return;a.ep=!0;const o=s(a);fetch(a.href,o)}})();const x=[{id:"northern-metropolis",title:"Northern Metropolis Spatial Analysis",period:"Sep 2025 - Dec 2025",tags:["GWR Analysis","Python"],color:"brand",icon:"fa-map",summary:"Performed GWR analysis on employment categories using integrated census and Foursquare POI data. Demonstrated superior explanatory power (R² > 0.5) and generated 2033 job distribution predictions.",fullDescription:`
    ...
    `).join(""))}function $(){const e=window.innerWidth-document.documentElement.clientWidth;e>0&&(document.body.style.paddingRight=`${e}px`,i&&(i.classList.add("no-transition"),i.style.paddingRight=`${e}px`)),document.body.style.overflow="hidden"}function E(){document.body.style.overflow="",document.body.style.paddingRight="",i&&(i.style.paddingRight="",setTimeout(()=>{i.classList.remove("no-transition")},10))}function w(e){const t=x.find(s=>s.id===e);!t||!b||!r||(b.innerHTML=`
        <div class="max-w-4xl mx-auto">
            <div class="mb-12">
                <div class="flex flex-wrap gap-2 mb-4">
                    ${t.tags.map(s=>`<span class="px-3 py-1 bg-${t.color}-50 dark:bg-${t.color}-900/30 text-${t.color}-600 dark:text-${t.color}-400 text-xs font-bold rounded-full">${s}</span>`).join("")}
                </div>
                ...
                <div class="space-y-8">
                    <div class="p-6 rounded-2xl bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ${t.technologies.map(s=>`<span class="px-2 py-1 bg-white dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs font-medium rounded border border-slate-200 dark:border-slate-600">${s}</span>`).join("")}
                        </div>
                    </div>
                    ...
                    ${Object.keys(t.links).length>0?`
                        <div class="p-6 rounded-2xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-sm">
                            ...
                            <div class="space-y-3">
                                ${Object.entries(t.links).map(([s,d])=>`
                                    <a href="${d}" class="flex items-center justify-between group p-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                                        <span class="text-sm font-medium text-slate-600 dark:text-slate-400 capitalize">${s}</span>
    ...
    `,r.classList.remove("hidden"),setTimeout(()=>{r.classList.add("opacity-100"),$()},10))}function f(){r&&(r.classList.remove("opacity-100"),setTimeout(()=>{r.classList.add("hidden"),E(),window.location.hash.startsWith("#project/")&&history.pushState("",document.title,window.location.pathname+window.location.search)},500))}window.addEventListener("hashchange",()=>{const e=window.location.hash;if(e.startsWith("#project/")){const t=e.replace("#project/","");w(t)}else(e==="#projects"||e==="")&&f()});if(window.location.hash.startsWith("#project/")){const e=window.location.hash.replace("#project/","");w(e)}h==null||h.addEventListener("click",f);window.addEventListener("keydown",e=>{e.key==="Escape"&&!(r!=null&&r.classList.contains("hidden"))&&f()});S();
