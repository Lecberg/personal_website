(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))d(i);new MutationObserver(i=>{for(const s of i)if(s.type==="childList")for(const p of s.addedNodes)p.tagName==="LINK"&&p.rel==="modulepreload"&&d(p)}).observe(document,{childList:!0,subtree:!0});function a(i){const s={};return i.integrity&&(s.integrity=i.integrity),i.referrerPolicy&&(s.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?s.credentials="include":i.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function d(i){if(i.ep)return;i.ep=!0;const s=a(i);fetch(i.href,s)}})();const v=[{id:"northern-metropolis",title:"Northern Metropolis Spatial Analysis",period:"Sep 2025 - Dec 2025",tags:["GWR Analysis","Python"],color:"brand",icon:"fa-map",summary:"Performed GWR analysis on employment categories using integrated census and Foursquare POI data. Demonstrated superior explanatory power (R² > 0.5) and generated 2033 job distribution predictions.",fullDescription:`
    ...
    `,technologies:["VISSIM","Synchro","Python (for data analysis)","Signal 4.0"],role:"Simulation Analyst",challenges:"Calibrating the simulation model to accurately reflect local driving behavior and saturation flow rates.",images:[],links:{}}],u=document.getElementById("theme-toggle"),h=document.getElementById("theme-toggle-mobile"),g=document.documentElement,w=()=>{g.classList.toggle("dark"),g.classList.contains("dark")?localStorage.theme="dark":localStorage.theme="light"};localStorage.theme==="dark"||!("theme"in localStorage)&&window.matchMedia("(prefers-color-scheme: dark)").matches?g.classList.add("dark"):g.classList.remove("dark");u==null||u.addEventListener("click",w);h==null||h.addEventListener("click",w);const o=document.getElementById("navbar");window.addEventListener("scroll",()=>{window.scrollY>50?(o==null||o.classList.add("bg-white/90","dark:bg-slate-900/90","backdrop-blur-md","shadow-sm","py-2"),o==null||o.classList.remove("py-4","bg-transparent")):(o==null||o.classList.add("py-4","bg-transparent"),o==null||o.classList.remove("bg-white/90","dark:bg-slate-900/90","backdrop-blur-md","shadow-sm","py-2"))});const l=document.getElementById("menu-btn"),n=document.getElementById("mobile-menu"),L=document.querySelectorAll(".mobile-link");let c=!1;l==null||l.addEventListener("click",()=>{c=!c,c?(n==null||n.classList.add("open"),l.innerHTML='<i class="fa-solid fa-xmark text-2xl"></i>'):(n==null||n.classList.remove("open"),l.innerHTML='<i class="fa-solid fa-bars text-2xl"></i>')});L.forEach(e=>{e.addEventListener("click",()=>{c=!1,n==null||n.classList.remove("open"),l&&(l.innerHTML='<i class="fa-solid fa-bars text-2xl"></i>')})});const S=document.querySelectorAll("section"),I=document.querySelectorAll(".nav-link");window.addEventListener("scroll",()=>{let e="";S.forEach(t=>{const a=t.offsetTop;window.pageYOffset>=a-200&&(e=t.getAttribute("id")||"")}),I.forEach(t=>{var a;t.classList.remove("active","text-brand-600"),(a=t.getAttribute("href"))!=null&&a.includes(e)&&t.classList.add("active","text-brand-600")})});const b=document.getElementById("projects-grid"),r=document.getElementById("project-overlay"),x=document.getElementById("overlay-content"),f=document.getElementById("close-overlay"),m=document.querySelector("#navbar .container");function $(){b&&(b.innerHTML=v.map(e=>`
    ...
    `).join(""))}function E(){const e=window.innerWidth-document.documentElement.clientWidth;e>0&&(document.body.style.paddingRight=`${e}px`,m&&(m.style.paddingRight=`${e}px`)),document.body.style.overflow="hidden"}function T(){document.body.style.overflow="",document.body.style.paddingRight="",m&&(m.style.paddingRight="")}function k(e){const t=v.find(a=>a.id===e);!t||!x||!r||(x.innerHTML=`
    ...
    `,r.classList.remove("hidden"),setTimeout(()=>{r.classList.add("opacity-100"),E()},10))}function y(){r&&(r.classList.remove("opacity-100"),setTimeout(()=>{r.classList.add("hidden"),T(),window.location.hash.startsWith("#project/")&&history.pushState("",document.title,window.location.pathname+window.location.search)},500))}window.addEventListener("hashchange",()=>{const e=window.location.hash;if(e.startsWith("#project/")){const t=e.replace("#project/","");k(t)}else(e==="#projects"||e==="")&&y()});if(window.location.hash.startsWith("#project/")){const e=window.location.hash.replace("#project/","");k(e)}f==null||f.addEventListener("click",y);window.addEventListener("keydown",e=>{e.key==="Escape"&&!(r!=null&&r.classList.contains("hidden"))&&y()});$();
